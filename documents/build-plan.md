...
| #   | Task                          | Description                                                                                                                                            |
| --- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 3.1 | CDK init                      | Set up `cdk.json`, `bin/app.ts`, install CDK dependencies                                                                                              |
| 3.2 | StorageStack                  | S3 bucket (versioned, private, stage-parameterized name)                                                                                               |
| 3.3 | DatabaseStack                 | DynamoDB Accounts table (PK: accountId) + ApiKeys table (PK: keyId, GSI on accountId)                                                                  |
| 3.4 | ApiStack -- Lambda functions  | Create Lambda functions for user endpoints (list-entities, list-agents, get-agent, get-section) and admin endpoints (create-account, create-api-key, create-entity, create-agent). Wire IAM permissions to S3 + DynamoDB. |
| 3.5 | ApiStack -- API Gateway       | REST API with `/api/*` and `/admin/*` route trees, Lambda proxy integrations                                                                           |
